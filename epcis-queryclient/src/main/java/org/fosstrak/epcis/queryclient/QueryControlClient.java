...
public class QueryControlClient implements QueryControlInterface, X509TrustManager {
     ...
     * QueryControlInterface service method.
     *
     * @param url                   the URL of the repository, or <code>null</code> if a default
    ...
    public QueryControlClient(String url, Object[] authenticationOptions) {
        ...
        try {
            ...
            throw new RuntimeException("无法配置 QueryControlClient: " + e.getMessage(), e);
        }
    }
    ...
    private Properties loadProperties() {
        ...
        if (is != null) {
            try {
                ...
                System.out.println("无法从 " + QueryControlClient.class.getResource(PROPERTY_FILE).toString() + " 加载配置文件. 加载默认配置.");
            }
        } else {
     ...
     *
     * @param endpointAddress       The endpoint address this client will communicate to.
     * @param authenticationOptions The authentication options:
     *                              <p/>
     *                              <table border="1">
     *                              <tr>
        ...
        if (authenticationOptions != null) {
            if (AuthenticationType.BASIC.equals(authenticationOptions[0])) {
                ...
                if (isEmpty(username) || isEmpty(password)) {
                    throw new Exception("授权方法 " + authenticationOptions[0]
                            + " 需要正确的用户名和密码");
                }
    ...
    public List<String> getQueryNames() throws ImplementationExceptionResponse, SecurityExceptionResponse,
        ...
        if (!serviceConfigured) {
            throw new QueryClientNotConfiguredException(
                    "请通过调用 configureService(URL, String[]) 来配置服务.");
        }
     ...
     *
     * @param query The query in its XML form.
     * @return The QueryResults as it is returned from the repository's Query
     *         Operations Module.
     * @throws ServiceException               If an error within the Query Operations Module occurred.
     * @throws IOException
     ...
     *
     * @param queryStream The query in its XML form.
     * @return The QueryResults as it is returned from the repository's Query
     *         Operations Module.
     * @throws RemoteException                If an error communicating with the Query Operations Module
     *                                        occurred.
     * @throws ServiceException               If an error within the Query Operations Module occurred.
     * @throws QueryParameterExceptionResponse
     *
