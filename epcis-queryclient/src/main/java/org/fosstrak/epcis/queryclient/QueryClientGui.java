...
public class QueryClientGui extends WindowAdapter implements ActionListener, AuthenticationOptionsChangeListener {
    ...
    private void generateParamHashMap() {
        ...
        newEntry.setDescription("请指定事件发生时间, i.e. YYYY-MM-DDThh:mm:ss.sssZ");
    ...
    private void drawMainWindow(String queryUrl) {
        ...
        mainWindow = new JFrame("EPCIS 查询接口演示");
        ...
        isSubscribed = new JCheckBox("订阅查询");
        ...
        isSubscribed.addActionListener(new ActionListener() {
            public void actionPerformed(final ActionEvent e) {
                if (mwSubscriptionPanel.isVisible()) {
                    ...
                    mwRunQueryButton.setText("查询");
                    mainWindow.pack();
                } else {
                    mwSubscriptionPanel.setVisible(true);
                    mwRunQueryButton.setText("订阅查询");
                    mainWindow.pack();
                }
            }
        });
        ...
        mwSubscribeManagementPanel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createTitledBorder("订阅管理"), BorderFactory.createEmptyBorder(5, 5, 5, 5)));

        mwEventTypeSelectPanel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createTitledBorder("返回事件"), BorderFactory.createEmptyBorder(5, 5, 5, 5)));

        mwQueryPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("查询参数"),
                BorderFactory.createEmptyBorder(5, 5, 5, 5)));

        mwSubscriptionPanel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createTitledBorder("订阅参数"), BorderFactory.createEmptyBorder(5, 5, 5, 5)));

        mwServiceUrlLabel = new JLabel("查询接口 URL: ");
        mwServiceInfoButton = new JButton("信息");
        mwServiceInfoButton.addActionListener(this);
        mwAuthOptions = new AuthenticationOptionsPanel(this);

        mwUnsubscribeQueryLabel = new JLabel("取消订阅 ID: ");
        mwUnsubscribeQueryTextField = new JTextField("", 40);
        mwUnsubscribeQueryTextField.setToolTipText("唯一的订阅 ID");
        mwUnsubscribeQueryButton = new JButton("取消订阅");
        mwUnsubscribeQueryButton.addActionListener(this);
        mwSubscriptionIdButton = new JButton("显示所有订阅的 ID");
        ...
        c.gridx = 1;
        c.gridy = 0;
        triggerIf = new JCheckBox("用触发器代替计划");
        ...
        c.gridx = 0;
        c.gridy = 1;
        JLabel mwDestUri = new JLabel("目标 URI: ");
        ...
        c.gridy = 2;
        c.gridwidth = 1;
        JLabel mwInitRecTime = new JLabel("初始化时间: ");
        ...
        c.gridy = 3;
        c.gridwidth = 1;
        JLabel mwSubId = new JLabel("订阅 ID: ");
        ...
        c.gridy = 4;
        c.gridwidth = 1;
        JLabel mwSchedule = new JLabel("计划: ");
        mwSubscriptionPanel.add(mwSchedule, c);

        c.fill = GridBagConstraints.NONE;
        c.anchor = GridBagConstraints.WEST;
        c.gridx = 1;
        JLabel mwScheduleDay = new JLabel("日: ");
        ...
        c.gridx = 3;
        c.insets = new Insets(10, 5, 5, 0);
        JLabel mwScheduleMonth = new JLabel("月: ");
        ...
        c.gridx = 5;
        c.insets = new Insets(10, 5, 5, 0);
        JLabel mwScheduleWeek = new JLabel("周几: ");
        ...
        c.gridx = 1;
        c.insets = new Insets(10, 5, 5, 0);
        JLabel mwScheduleHour = new JLabel("时: ");
        ...
        c.gridx = 3;
        c.insets = new Insets(10, 5, 5, 0);
        JLabel mwScheduleMinute = new JLabel("分: ");
        ...
        c.gridx = 5;
        c.insets = new Insets(10, 5, 5, 0);
        JLabel mwScheduleSec = new JLabel("秒: ");
        ...
        mwRunQueryButton = new JButton("执行查询");
    ...
    private void mwSubscriptionIdButtonPressed() {
        try {
            ...
            dwOutputTextArea.append("查询不到任何订阅信息.");
    ...
    private void mwUnsubscribeQueryButtonPressed(String subscriptionID) {
        try {
            ...
            if (subscriptionID.equals("")) {
                JOptionPane.showMessageDialog(frame, "请指定要取消的订阅 ID", "服务调用成功",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            client.unsubscribe(subscriptionID);
            JOptionPane.showMessageDialog(frame, "取消订阅成功.", "服务调用成功",
                    JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception ex) {
            dwOutputTextArea.append("不能取消订阅.");
            StringWriter detailed = new StringWriter();
            PrintWriter pw = new PrintWriter(detailed);
            ex.printStackTrace(pw);
            dwOutputTextArea.append(detailed.toString());
        ...
        try {
            ...
            if (isSubscribed.isSelected()) {
                if (mwSubIdField.getText().equals("")) {
                    JFrame frame = new JFrame();
                    JOptionPane.showMessageDialog(frame, "请指定订阅 ID", "输入参数错误",
                ...
                debug("查询参数个数: " + queryParams.getParam().size() + "\n");
                ...
                JFrame frame = new JFrame();
                JOptionPane.showMessageDialog(frame, "订阅查询成功.", "服务调用成功",
                        JOptionPane.INFORMATION_MESSAGE);
            } else {
                data = runQuery();
                createResultsWindow();
    ...
    private void showErrorFrame() {
        ...
        String msg = "调用查询接口时发现未捕获的异常.\n"
                + "详细情况请查看 Debug 窗口.";
        JOptionPane.showMessageDialog(frame, msg, "服务调用失败", JOptionPane.ERROR_MESSAGE);
    }
    ...
    private void mwInfoButtonPressed() {
        ...
        try {
            ...
            String text = "响应信息:\n" + "应用版本: " + standardVersion + "\n"
                    + "服务版本: " + vendorVersion + "\n" + "支持的查询: ";
            for (String elem : queryNames) {
                text += elem + "\n";
            }
            JFrame frame = new JFrame();
            JOptionPane.showMessageDialog(frame, text, "服务调用成功", JOptionPane.INFORMATION_MESSAGE);

        } catch (Exception e) {
            dwOutputTextArea.append("无法执行的查询:");
            StringWriter detailed = new StringWriter();
            PrintWriter pw = new PrintWriter(detailed);
            e.printStackTrace(pw);
            dwOutputTextArea.append(detailed.toString());
    ...
    private void examplesChanged() {
        ...
        if (selected >= 0) {
            ...
            for (QueryParam item : ex.getQueryParameters()) {
                ...
                if (toAddItem == null) {
                    dwOutputTextArea.append("Bug: 查询条件中使用了不支持的查询参数");
                } else {
